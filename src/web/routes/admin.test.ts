...
// Real implementation (not a passthrough stub), with `run`/`runMany` wrapped in spies so the
...
const { mockQueueRun, mockQueueRunMany } = vi.hoisted(() => ({ mockQueueRun: vi.fn(), mockQueueRunMany: vi.fn() }));
...
vi.mock('../../shared/mutationQueue', async () => {
  ...
  return {
    createMutationQueue: <K = string>() => {
      ...
      return { ...queue, run: mockQueueRun, runMany: mockQueueRunMany };
    },
  };
});
...
describe('POST /users/update', () => {
  ...
  // then called runUserMutationForActorAndTarget (so checkOrder < runOrder); the fixed version
  // enters the queue first and the check runs as part of the queued callback (so runOrder <
  // checkOrder). Compares mock.invocationCallOrder from a single request — no timing/concurrency
  // simulation needed. The guarded actor+target path now goes through `runMany`, not `run`.
