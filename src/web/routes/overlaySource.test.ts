...
describe('GET /videos/:streamerId/:filename', () => {
  ...
  it('returns 404 without sending when the real path escapes the folder via a symlink', async () => {
    ...
    expect(res.status).toBe(404);
    expect(realPathWithin).toHaveBeenCalledWith('/app/overlay-videos', '/app/overlay-videos/123/clip.mp4');
  ...
  it('sends the resolved real path rather than the lexical one', async () => {
    vi.mocked(realPathWithin).mockResolvedValueOnce('/app/overlay-videos/real/clip.mp4');
    const sendFileSpy = vi.fn();
    const app = express();
    app.use((req, res, next) => {
      (res as any).sendFile = (filePath: string) => { sendFileSpy(filePath); res.end(); };
    ...
    const res = await supertest(app).get('/videos/123/clip.mp4');
    expect(res.status).toBe(200);
    expect(sendFileSpy).toHaveBeenCalledWith('/app/overlay-videos/real/clip.mp4');
  });
